...
const initialSettings = getStoreSettings() ?? {
  store_name: '',
  logo_url: '',
  hero_image_url: '',
  tagline: '',
          ...
          <TabsContent value="products">
            <Card>
              ...
              <CardContent className="space-y-4">
                {products.length === 0 && <p className="text-neutral-500 py-8 text-center">Nenhum produto cadastrado</p>}
                {products.map(p => (
                  <div key={p.id} className="flex gap-4 border border-neutral-100 p-4 rounded-lg">
                    <div className="w-16 h-20 bg-neutral-100 rounded flex items-center justify-center flex-shrink-0 overflow-hidden">
                      {p.image_url ? <img src={p.image_url} alt={p.name} className="object-cover w-full h-full" /> : <Image size={20} className="text-neutral-400" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{p.name}</p>
                      <p className="text-sm text-neutral-500">R$ {p.price.toFixed(2).replace('.', ',')}</p>
                      {p.stock !== undefined && <p className="text-xs text-neutral-400">Estoque: {p.stock}</p>}
                    </div>
