...
function loadData() {
  ...
  return {
    settings: s ?? ({ store_name: 'Minha Loja' } as StoreSettings),
    products: getProducts().filter(p => p.featured),
...
export default function Home() {
  ...
  const [settings] = useState<StoreSettings>(initial.settings)
  const [products] = useState<Product[]>(initial.products)
  const [cartItems] = useState<CartItem[]>(initial.cartItems)
