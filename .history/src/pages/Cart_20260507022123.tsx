...
export default function Cart({ initialCartItems = [], storeSettings }: CartProps) {
  ...
  const handleWhatsAppOrder = () => {
    ...
    cartItems.forEach(item => {
      message += `• ${item.product_name} - Tam: ${item.size} - Qtd: ${item.quantity} - R$ ${(item.price * item.quantity).toFixed(2).replace('.', ',')}\n`
    })
    message += `\n*Total: R$ ${total.toFixed(2).replace('.', ',')}*`

    const whatsappUrl = `https://wa.me/${storeSettings.whatsapp}?text=${encodeURIComponent(message)}`
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col">
                      <div className="flex-1">
                        <h3 className="font-medium text-neutral-900 mb-1">{item.product_name}</h3>
                        <p className="text-sm text-neutral-500">Tamanho: {item.size}</p>
                        <p className="text-sm text-neutral-500">Quantidade: {item.quantity}</p>
                      </div>
